...
const fillEmptyValues = data => {
  ...
  for (const key in clone) {
    if (lodashEs.isPlainObject(clone[key])) {
...
const unsetCompact = (data, path) => {
  ...
  if (sanitizedPath.includes('[]')) {
    ...
    const arr = lodashEs.get(data, startPath);
  ...
  if (sanitizedPath.charAt(sanitizedPath.length - 1) === ']') {
    ...
    const arr = lodashEs.get(data, sanitizedPath.slice(0, sanitizedPath.lastIndexOf('[')));
    if (Array.isArray(arr) && match?.groups?.index !== undefined) {
      arr.splice(Number(match.groups.index), 1);
    }
  } else {
    lodashEs.unset(data, sanitizedPath);
  }
};

const useMaybeRemember = (initialValue, rememberKey) => {
  const [rememberedData, setRememberedData] = react.useRemember(initialValue, rememberKey);
  const [localData, setLocalData] = React.useState(initialValue);
  if (rememberKey) {
...
const FormMetaWrapper = ({
  ...
  const [nestedAttributes, addAttribute] = React.useReducer(attributesReducer, new Set());
  const metaValues = {
    nestedAttributes,
    addAttribute,
    model,
  ...
  return jsxRuntime.jsx(FormMetaProvider, {
    value: metaValues,
    children: children
  });
};
...
function useInertiaForm(rememberKeyOrInitialValues, maybeInitialValues) {
  const getFormArguments = React.useCallback(() => {
  ...
  const [defaults, setDefaults] = React.useState(transformedData || {});
  const [data, setData] = useMaybeRemember(transformedData, rememberKey ? `${rememberKey}:data` : undefined);
  const rootModelKey = React.useMemo(() => {
    const keys = data ? Object.keys(data) : [];
    if (keys.length === 1 && lodashEs.isPlainObject(data[keys[0]])) {
      return keys[0];
    }
    return undefined;
  }, [data]);
  ...
  const recentlySuccessfulTimeoutId = React.useRef(null);
  let transformRef = React.useRef(data => data);
  ...
  let onChangeRef = React.useRef(null);
  let onChangeArgsRef = React.useRef(null);
  React.useEffect(() => {
  ...
  const submit = (method, url, options = {
    ...
    if (options.async === true) {
      ...
      if (method === 'delete') {
        react.router.delete(url, {
          ..._options,
          data: transformedData
        });
      } else {
        react.router[method](url, transformedData, _options);
      }
    }
  };
  ...
  return {
    data,
    isDirty: !lodashEs.isEqual(data, defaults),
    errors,
    hasErrors,
    processing,
    progress,
      ...
      if (typeof keyOrData === 'string') {
        return setData(data => {
          ...
          if (onChangeRef.current) {
            onChangeArgsRef.current = [keyOrData, maybeValue, lodashEs.get(data, keyOrData)];
          }
          lodashEs.set(clone, keyOrData, maybeValue);
          return clone;
        });
      }
    ...
    getData: key => {
      return lodashEs.get(data, key);
    },
    unsetData: key => {
      setData(data => {
        const clone = structuredClone(data);
        if (onChangeRef.current) {
          onChangeArgsRef.current = [key, lodashEs.get(data, key), undefined];
        }
    ...
    reset: fields => {
      ...
      arrFields.forEach(field => {
        lodashEs.set(clone, field, lodashEs.get(defaults, field));
      });
      clearErrors(fields);
      if (onChangeRef.current) {
        onChangeArgsRef.current = [undefined, clone, data];
    ...
    getError: key => {
      return lodashEs.get(errors, key);
    },
    clearErrors,
    submit,
    get: (url, options) => {
...
const Form = ({
  ...
  const filteredData = React.useCallback(data => {
  ...
  const contextValueObject = React.useCallback(() => ({
    ...form,
    model,
    method,
    to,
...
const WrappedForm = ({
  ...
  return jsxRuntime.jsx(FormMetaWrapper, {
    model: model,
...
const NestedFields = ({
  ...
  return jsxRuntime.jsx(NestedAttributeProvider, {
    value: inputModel,
...
const Submit = /*#__PURE__*/React.forwardRef(({
  ...
  const hasEmptyRequiredFields = React.useCallback(() => {
...
const useDynamicInputs = ({
  ...
  const handleAddInputs = React.useCallback(override => {
    setData(formData => {
      const clone = structuredClone(formData);
      let node = lodashEs.get(clone, inputModel);
      if (!node || !Array.isArray(node)) {
        lodashEs.set(clone, inputModel, []);
        node = lodashEs.get(clone, inputModel);
      }
      let merge = {};
      ...
      lodashEs.set(clone, inputModel, node);
      return clone;
    });
  }, []);
  const handleRemoveInputs = React.useCallback(i => {
    const record = getData(`${inputModel}[${i}]`);
    unsetData(`${inputModel}[${i}]`);
    return record;
  }, []);
  const data = getData(inputModel);
  const generatePaths = React.useCallback(() => {
...
const DynamicInputs = ({
  ...
  model,
  emptyData,
  addInputButton = jsxRuntime.jsx("button", {
    children: "+"
  }),
  removeInputButton = jsxRuntime.jsx("button", {
    children: "-"
  })
}) => {
  const {
  ...
  return jsxRuntime.jsxs(jsxRuntime.Fragment, {
    children: [/*#__PURE__*/React.cloneElement(addInputButton, {
      onClick: () => addInput(),
      type: 'button'
    }), paths.map((path, i) => jsxRuntime.jsxs(NestedFields, {
      model: path,
...
exports.DynamicInputs = DynamicInputs;
exports.Form = WrappedForm;
exports.Input = Input;
exports.NestedFields = NestedFields;
exports.Submit = Submit$1;
exports.useDynamicInputs = useDynamicInputs;
exports.useForm = useForm;
exports.useInertiaForm = useInertiaForm;
exports.useInertiaInput = useInertiaInput;
